...
export const productionContext: Record<string, ProductionContext> = {
  ...
  "schoenberg.completing-sentence.d": {
    why: "A first complete attempt gives you something concrete to compare with Schoenberg's source examples.",
    when: "Use it before detailed repertoire analysis so later examples can revise an actual compositional decision rather than remain abstract.",
    tools: ["Piano roll", "Staff notation", "Scale degrees", "Roman numerals"],
    visual: "motif",
    realWorld: "Build the full MIDI phrase first, then use reference examples to diagnose whether its continuation and ending are doing enough.",
  },
  "schoenberg.completing-sentence.e": {
    why: "Ex. 52 makes the complete process unusually visible: establishment, reduced motive-forms, liquidation and cadence.",
    when: "Use it as a reference model when a sentence feels like two unrelated halves.",
    tools: ["Native score", "Source playback", "Formal annotations"],
    visual: "motif",
    realWorld: "Mark formal stages directly over a MIDI or notation sketch and listen to how each stage changes the material's behaviour.",
  },
  "schoenberg.completing-sentence.f": {
    why: "Remote motive-forms may need repetition or sequence before the listener can understand their relation to the source.",
    when: "Use this when a developmental continuation feels disconnected even though its material is theoretically derived.",
    tools: ["Native score", "Sequence", "Loop playback"],
    visual: "motif",
    realWorld: "Repeat a transformed fragment once or twice at new pitch levels before pushing it into a more condensed version.",
  },
  "schoenberg.completing-sentence.g": {
    why: "Unequal spans show that sentence function is not identical with an eight-bar grid.",
    when: "Use this when a phrase works musically but feels forced by symmetrical bar-count expectations.",
    tools: ["Native score", "Formal spans", "Loop playback"],
    visual: "arrangement",
    realWorld: "Move phrase boundaries in the timeline and judge function by musical behaviour rather than by equal clip lengths.",
  },
  "schoenberg.completing-sentence.h": {
    why: "Inserted repetition can explain apparent irregularity as an intelligible extension rather than an arbitrary extra bar.",
    when: "Use it when a continuation needs more time to establish a transformed idea.",
    tools: ["Native score", "Loop region", "Formal markers"],
    visual: "arrangement",
    realWorld: "Duplicate a derived cell inside the continuation and compare the extended and unextended forms back to back.",
  },
  "schoenberg.completing-sentence.i": {
    why: "Extension can lengthen the continuation without changing its forward formal function.",
    when: "Use it when the material needs more developmental space before the cadence.",
    tools: ["Native score", "Piano roll", "Cadential markers"],
    visual: "motif",
    realWorld: "Extend a developmental clip while keeping the opening fixed, then check that the extra material still increases direction toward the close.",
  },
  "schoenberg.completing-sentence.j": {
    why: "Ex. 61 connects developing variation and liquidation as stages of one continuous transformational chain.",
    when: "Use it when liquidation feels pasted on rather than grown from the continuation.",
    tools: ["Native score", "Motive annotations", "Loop playback"],
    visual: "motif",
    realWorld: "Make each successive MIDI fragment derive from the previous one, then let the late versions shed detail instead of switching abruptly to cadential filler.",
  },
  "schoenberg.completing-sentence.k": {
    why: "Comparing different source solutions prevents one model from becoming a rigid formula.",
    when: "Use it before final revision to separate common function from example-specific surface details.",
    tools: ["Source playback", "A/B comparison", "Formal annotations"],
    visual: "reference",
    realWorld: "Reference several passages rather than one template and note what they share functionally before revising your own phrase.",
  },
  "schoenberg.completing-sentence.l": {
    why: "The final revision applies source-derived principles without copying one source surface.",
    when: "Use it to finish the sentence before moving on to the period.",
    tools: ["Piano roll", "Staff notation", "Source reference", "Loop playback"],
    visual: "motif",
    realWorld: "Revise the continuation after reference listening, keeping source identity while choosing your own sequence, extension and liquidation route.",
  },
