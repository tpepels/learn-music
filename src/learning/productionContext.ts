...
export const productionContext: Record<string, ProductionContext> = {
  ...
  "harmony.chords.b": {
    why: "Putting I, IV, and V under an existing groove and melody turns function into something audible: the chords change the direction of a phrase rather than merely filling a chord lane.",
    when: "During songwriting, once a rhythmic or melodic idea exists and harmony has to support its shape.",
    tools: ["Chord track", "Loop playback", "Drum groove", "Melody clip"],
    visual: "harmony",
    realWorld: "Producers commonly loop drums and melody while trying chord changes underneath, because function is easier to judge in the full phrase than as isolated piano chords.",
  },
  "harmony.chords.c": {
    why: "Comparing V-I with V-vi makes closure audible. The listener hears the same dominant expectation either fulfilled by tonic or redirected into a deceptive continuation.",
    when: "At phrase endings and section boundaries, when the writer needs to decide whether the music should close or keep moving.",
    tools: ["Chord track", "Loop playback", "A/B comparison", "Bass movement"],
    visual: "harmony",
    realWorld: "A common DAW workflow is to duplicate or loop a phrase and change only its final chord so two endings can be compared without changing the groove or melody.",
  },
  "harmony.chords.d": {
    why: "Chord identity and accompaniment texture are separate decisions. The same harmony can be blocked, pulsed, broken, or arpeggiated while keeping the same harmonic function.",
    when: "After a progression works harmonically and the writer needs to turn chord symbols into an actual keyboard, guitar, synth, or ensemble part.",
    tools: ["Chord track", "MIDI editor", "Arpeggiator", "Loop playback"],
    visual: "harmony",
    realWorld: "In a DAW, the chord label may stay C while its MIDI clip changes from one vertical stack into repeated or staggered chord tones.",
  },
