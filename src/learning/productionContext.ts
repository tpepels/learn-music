...
export const productionContext: Record<string, ProductionContext> = {
  ...
  "schoenberg.completing-sentence.d": {
    ...
    when: "Use this as a first theme-writing workflow before comparing your solution with Schoenberg's literature examples.",
  ...
  "schoenberg.completing-sentence.e": {
    why: "Ex. 52 makes shortening and liquidation visible as a continuous process from full phrase-forms to residues.",
    when: "Use this model when a continuation feels as though it keeps restarting instead of progressively approaching an ending.",
    tools: ["Source analysis", "Staff notation", "Loop playback", "Formal markers"],
    visual: "motif",
    realWorld: "Duplicate a derived phrase, then shorten successive copies in the MIDI editor while keeping enough contour or rhythm for the derivation to remain audible.",
  },
  "schoenberg.completing-sentence.f": {
    why: "Remote motive-forms may need repetition simply to become comprehensible, so extra length can have a musical reason.",
    when: "Use this when a new derivative sounds abrupt on first appearance or when you are deciding whether a longer continuation is justified.",
    tools: ["Loop playback", "Motive labels", "Arrangement markers", "Piano roll"],
    visual: "motif",
    realWorld: "Introduce a more distant variation, repeat or restate it once, and A/B the longer version against a single unexplained appearance.",
  },
  "schoenberg.completing-sentence.g": {
    why: "Progressive variation can create the pattern that later drives a sequence, linking local motivic work to larger directional motion.",
    when: "Use this when a sequence sounds pasted on rather than generated by the preceding material.",
    tools: ["MIDI transpose", "Motif labels", "Roman numerals", "Loop playback"],
    visual: "motif",
    realWorld: "Develop a short source through two or three stages, then use the latest derivative as the repeated/transposed sequence pattern.",
  },
  "schoenberg.completing-sentence.h": {
    why: "Real repertoire often bends the eight-measure practice form while preserving the same establishment-continuation-cadence logic.",
    when: "Use this when your phrase length is irregular but you want to know whether the irregularity is formally intelligible.",
    tools: ["Arrangement markers", "Staff notation", "Loop playback", "Phrase labels"],
    visual: "arrangement",
    realWorld: "Mark formal functions across the timeline before judging bar count; unequal section lengths are acceptable if each span has a clear job.",
  },
  "schoenberg.completing-sentence.i": {
    why: "Schoenberg's omission test reveals whether extra measures are genuine structural necessities or interpolated extensions.",
    when: "Use this when a theme feels too long and you cannot tell which measures are responsible.",
    tools: ["DAW duplicate", "Mute", "Arrangement markers", "A/B playback"],
    visual: "arrangement",
    realWorld: "Duplicate the phrase, mute suspected inserted bars in one version, and compare whether the simpler structural span becomes clearer.",
  },
  "schoenberg.completing-sentence.j": {
    why: "Exceptional harmonic or formal events can be analysed without being promoted into default compositional recipes.",
    when: "Use this when studying an unusual masterwork passage or considering a deliberately unconventional beginning or cadence.",
    tools: ["Roman numerals", "Staff notation", "A/B playback", "Reference analysis"],
    visual: "harmony",
    realWorld: "Annotate the unusual event in the score or DAW and compare it against a safer conventional alternative before deciding which effect you actually need.",
  },
  "schoenberg.completing-sentence.k": {
    why: "Developing variation can move through different voices and increasingly remote forms while liquidation later strips the material down again.",
    when: "Use this when a theme's surface changes dramatically but you want the motivic thread to remain traceable.",
    tools: ["Motive labels", "Voice lanes", "Piano roll", "Staff notation"],
    visual: "motif",
    realWorld: "Pass a derived figure between melody and accompaniment tracks, then reduce its final appearances to smaller residues before the cadence.",
  },
  "schoenberg.completing-sentence.l": {
    why: "The chapter's full method replaces a rigid bar-count formula with causal formal logic: establish, develop, justify extension, liquidate and close.",
    when: "Use this as the final revision pass on a sentence before moving on to period construction and larger forms.",
    tools: ["Piano roll", "Staff notation", "Arrangement markers", "Roman numerals"],
    visual: "motif",
